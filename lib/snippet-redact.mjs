 ...
 * point. Beyond it the result carries `fixedPoint: false` and one or more runs
 * may remain unredacted — see the `fixedPoint` note on redactSecretShapes.
 */
export const MAX_REDACTION_PASSES = 64;
...
export function redactSecretShapes(text) {
  if (typeof text !== "string" || text.length === 0) {
    return { text: text ?? "", shapes: [], fixedPoint: true };
  }
  let out = text;
  ...
  // trailing "=" blocks the lookahead until the second run is replaced). Each
  // changing pass peels one layer of adjacency, so N adjacent runs need N
  // passes. The cap guards against pathological oscillation; when it is
  // reached the text is NOT a fixed point, and `fixedPoint: false` says so
  ...
  return { text: out, shapes, fixedPoint };
}
