...
// extension. Placeholders (<x>), URLs, and glob-ish tokens are excluded.
const TOOL_REF_RE = /(?<![\w/.<>])((?:\.{1,2}\/)?(?:[\w.-]+\/)*[\w.-]+\.(?:mjs|cjs|ts|sh|py))(?![\w/])/g;

/**
 * Extract tool/script references from index text. Pure. Dedupes by token,
 * keeping the first line each appears on. Lines carrying a URL are skipped
 * whole (too false-positive-prone), as are `<placeholder>` tokens.
 * @param {string} text
 ...
 *   phantom-tool check does not run at all.
 ...
 *
 * Caller contract: `bytes` MUST be LF-normalized UTF-8 bytes. A raw
 * `statSync().size` on a Windows working tree reads +1 per line and would flip
 * agents over the cap for their line endings alone.
 *
 * @param {object} input
 ...
 *   missingCount: number, verdict: "clean"|"findings"|"nothing-swept"}}
 */
export function classifyMemoryIndexSizes({ rows, budgetBytes } = {}) {
  ...
  for (const raw of Array.isArray(rows) ? rows : []) {
    const agent = asString(raw?.agent) || "(unnamed agent)";
    const path = typeof raw?.path === "string" ? raw.path : null;
    const bytes = Number.isFinite(raw?.bytes) ? Number(raw.bytes) : null;

    if (raw?.dirExists === false) {
      out.push({
