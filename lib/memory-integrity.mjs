 ...
 *   phantom-tool          WARN  an index line names a tool/script that resolves
 *                               to no file (opt-in: needs a `toolResolver`)
...
/**
 * Default index load budget: **24,400 LF-normalized UTF-8 bytes**.
 *
 * ⚠ CORRECTED 2026-08-29. This read `Math.floor(24.4 * 1024)` = **24,985** —
 * the loader's documented "24.4KB" re-typed as KiB where the number is
 * decimal. The one check that can catch a truncating index carried 585 bytes
 * of slack it was never granted, in the only direction that matters: it let a
 * truncated index pass. Pinned to the literal, with a test at 24,401, so the
 * re-typing cannot come back.
 *
 * Measure the index in LF-normalized UTF-8 bytes. A raw `statSync().size` on a
 * Windows working tree reads +1 per line and flips an index over the cap for
 * its line endings alone.
 */
export const DEFAULT_INDEX_BUDGET_BYTES = 24_400;

/**
 * The sentence a budget finding should carry, so every surface (CLI, report,
 * dashboard) says the same thing about WHY the byte count matters: nothing
 * errors, nothing warns, the tail is simply gone.
 */
export const LOADER_TRUNCATION_SENTENCE = "the loader drops the tail silently past this line";
...
// ---- phantom-tool leg ----
// An index line NAMING a tool or script must resolve to an existing file. The
// live instance: an index named an "open-items projection" tool that never
// existed — a compression pass had kept the NAME and dropped the inline
// recipe, leaving the line unfollowable for eleven days. Every reader saw a
// capability the fleet did not have.
//
// Extraction is pure; EXISTENCE is judged by a caller-supplied resolver, so
// this file stays fs-free and the check stays testable without a disk.

// A tool-shaped token: optional relative path prefix + basename with a script
// extension, `/` or `\` separated. Placeholders (<x>), URL/URI schemes, and
// glob-ish tokens are excluded. A token preceded by `:` is skipped so that
// `file:foo.py` / `mailto:foo.py` do not shed their scheme and surface as a
// bare `foo.py` the resolver then "proves" absent — the extractor must never
// invent a token the index does not contain. Path segments are bounded: an
// unbounded `[\w.-]+` backtracks quadratically on a long line with no match
// (30,000 hyphens took 2.5 s), and an over-budget index is exactly the input
// this library exists to handle.
const TOOL_REF_RE =
  /(?<![\w/\\.<>:])((?:\.{1,2}[/\\])?(?:[\w.-]{1,128}[/\\])*[\w.-]{1,128}\.(?:mjs|cjs|ts|sh|py))(?![\w/\\])/g;
/** Linear pre-check: a line with no candidate extension never reaches the matcher. */
const TOOL_EXT_HINT_RE = /\.(?:mjs|cjs|ts|sh|py)(?![\w/\\])/;

/**
 * Extract tool/script references from index text. Pure. Dedupes by token,
 * keeping the first line each appears on. Lines carrying an http(s) URL are
 * skipped whole (too false-positive-prone), as are `<placeholder>` tokens.
 * Tokens are returned as written — `scripts\foo.py` stays `scripts\foo.py`,
 * never its basename — so the resolver judges the reference the index made.
 * @param {string} text
 * @returns {Array<{token: string, line: number}>}
 */
export function extractToolReferences(text) {
  if (typeof text !== "string" || text.length === 0) return [];
  const seen = new Map();
  const lines = text.split(/\r?\n/);
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (!TOOL_EXT_HINT_RE.test(line)) continue;
    if (/https?:\/\//.test(line)) continue; // URL-bearing line: skip entirely
    let m;
    TOOL_REF_RE.lastIndex = 0;
    while ((m = TOOL_REF_RE.exec(line)) !== null) {
      const token = m[1];
      if (token.includes("<") || token.includes(">")) continue;
      if (!seen.has(token)) seen.set(token, i + 1);
    }
  }
  return [...seen.entries()].map(([token, line]) => ({ token, line }));
}
 ...
 * @param {number} [input.indexByteLength]   LF-normalized UTF-8 byte length of
 *   `indexText` — NOT `statSync().size`, which on a CRLF working tree reads +1
 *   per line and puts a valid index over the cap for its line endings alone
 * @param {number} [input.budgetBytes]       load budget (default 24,400 bytes)
 * @param {(token: string) => boolean|null|undefined} [input.toolResolver]
 *   OPT-IN existence check for tool/script names in the index. `true` = exists,
 *   `false` = provably absent (the only value that yields a finding),
 *   `null`/`undefined`/throw = can't judge → NO finding. Omit it and the
 *   phantom-tool check does not run at all. MUST be synchronous: a Promise is
 *   never `false`, so an async resolver would make the check permanently
 *   inert while looking enabled — that is a caller error, not a data
 *   ambiguity, and it throws a TypeError rather than failing soft.
 * @returns {{findings: Array<{type: string, severity: "warn"|"info", message: string, detail?: object}>, swept: boolean, coverage: {reached: number, skipped: number, malformed: number, contentless: number, indexExcluded: number, indexRead: boolean}}}
 *   `reached` = non-index entries with a string name AND string content (empty
 *   string counts — it was supplied). `malformed` = null, or a missing/non-string
 *   name. `contentless` = valid name, content not a string (an upstream read
 *   failed). `indexExcluded` = entries named MEMORY.md, case-insensitively.
 *   `skipped` = the sum of those three. `indexRead` = `indexText` was a non-empty
 *   string, which is not proof that any file I/O succeeded. `swept` =
 *   `reached > 0 || indexRead`.
 *
 ...
 *
 * `coverage` describes THE INVENTORY YOU SUPPLIED and nothing else. It has no
 * `expected` count and cannot have one - this function never touches a disk, so
 * it cannot know a file existed that your `readdir` missed. `reached` alone
 * cannot tell 3-of-3 from 3-of-400. To catch that quieter failure, compare
 * `reached` and the skip counts against an expected total you established
 * independently, at the call site that did the I/O. Subjects omitted before
 * invocation are invisible here by construction.
 */
export function lintMemoryIntegrity({ indexText, files, indexByteLength, budgetBytes, toolResolver } = {}) {
  const findings = [];
  ...
  // --- phantom tools ---
  // Only when the caller supplies a resolver; this file never touches a disk.
  // Resolver contract: true = exists, false = provably absent, anything else
  // (including a throw) = can't judge → NO finding. Fail-soft, as everywhere
  // here: the lint may under-report, never over-report.
  if (typeof toolResolver === "function") {
    for (const ref of extractToolReferences(indexText)) {
      let exists = null;
      try {
        exists = toolResolver(ref.token);
      } catch {
        exists = null;
      }
      if (exists && typeof exists.then === "function") {
        throw new TypeError(
          "lintMemoryIntegrity: toolResolver must be synchronous — a Promise is never `false`, " +
            "so an async resolver silently disables the phantom-tool check",
        );
      }
      if (exists === false) {
        findings.push({
          type: "phantom-tool",
          severity: "warn",
          message: `index:${ref.line} names \`${ref.token}\`, which resolves to no file — a compression pass can keep a tool's name and drop its recipe, leaving the line unfollowable`,
          detail: { token: ref.token, line: ref.line },
        });
      }
    }
  }
  ...
  if (Number.isFinite(indexByteLength) && indexByteLength > budget) {
    findings.push({
      ...
      severity: "warn",
      message: `index is ${indexByteLength} bytes (budget ${budget}) — ${LOADER_TRUNCATION_SENTENCE}; trim digest lines into topic files`,
...
/** Per-agent verdicts from `classifyMemoryIndexSizes`. */
export const MEMORY_SIZE_STATES = Object.freeze({
  OVER: "over",
  OK: "ok",
  MISSING: "missing",
  NO_MEMORY_DIR: "no-memory-dir",
});

/**
 * Classify ONE read of every agent's index size across a fleet.
 *
 * `lintMemoryIntegrity` answers "is this one memory dir healthy?". This answers
 * the fleet question, and it exists because the fleet question has a failure the
 * single-dir one doesn't: an agent whose index you could not measure. Reporting
 * that agent as under budget is how a sweep reports a confident clean over a
 * fleet that is not the fleet.
 *
 * So the four states are deliberately not three:
 *   NO_MEMORY_DIR  no memory dir at all — no session has ever run there.
 *                  DECLARED, never swept, never a finding. Keeps the
 *                  denominator honest without manufacturing a daily amber.
 *   MISSING        the dir exists but the index is absent or unreadable.
 *                  FAIL-CLOSED — sessions run here with no index at all, and
 *                  that is a finding, not an absence.
 *   OVER / OK      actually measured.
 *
 * `sweptCount` counts only OVER and OK. MISSING is a finding but not a
 * measurement, so it cannot prop up the denominator. A run that measured
 * nothing returns `verdict: "nothing-swept"` — never `"clean"`. Findings
 * dominate: a one-agent fleet whose index is MISSING has zero measured agents,
 * and calling that "nothing-swept" would bury the loudest instance of the exact
 * failure this function exists to catch.
 *
 * Inventory faults are counted in `malformedCount` and skipped — never
 * classified. A row that is not an object, has no non-empty string `agent`, or
 * repeats an `agent` already seen cannot be a clean measurement, and it cannot
 * be a MISSING finding either: MISSING asserts that a dir exists and sessions
 * run there, and nothing is known about a row with no identity. A duplicate is
 * the same fault from the other side — it would count one agent twice.
 *
 * Caller contract: `bytes` is a non-negative safe integer of LF-normalized
 * UTF-8 bytes. A raw `statSync().size` on a Windows working tree reads +1 per
 * line and would flip agents over the cap for their line endings alone.
 * Anything that is not such an integer — negative, fractional, NaN, Infinity —
 * is not a measurement and classifies as MISSING (fail-closed), because a `-1`
 * accepted as a byte count sits under budget with 24,401 bytes of headroom.
 * Zero IS a measurement: an empty index is under the cap. (That differs from
 * `lintMemoryIntegrity`'s `indexRead`, which asks whether there was anything
 * to lint — a different question.) A row carrying both `dirExists: false` and
 * a real `bytes` is contradictory; the measurement wins, because it is the
 * thing that was actually observed.
 *
 * @param {object} input
 * @param {Array<{agent: string, path?: string|null, bytes?: number|null, dirExists?: boolean}>} input.rows
 * @param {number} [input.budgetBytes] defaults to DEFAULT_INDEX_BUDGET_BYTES
 * @returns {{budgetBytes: number, rows: Array<object>, findings: Array<object>,
 *   sweptCount: number, declaredCount: number, overCount: number,
 *   missingCount: number, malformedCount: number,
 *   verdict: "clean"|"findings"|"nothing-swept"}}
 */
export function classifyMemoryIndexSizes({ rows, budgetBytes } = {}) {
  const budget =
    Number.isFinite(budgetBytes) && budgetBytes > 0 ? Math.floor(budgetBytes) : DEFAULT_INDEX_BUDGET_BYTES;
  const out = [];
  const findings = [];
  const seen = new Set();
  ...
  for (const raw of Array.isArray(rows) ? rows : []) {
    const agent = raw && typeof raw === "object" && typeof raw.agent === "string" ? raw.agent.trim() : "";
    ...
    if (raw.dirExists === false && bytes === null) {
      out.push({
        agent, path, bytes: null,
        state: MEMORY_SIZE_STATES.NO_MEMORY_DIR,
        overBy: null, headroom: null,
        message: `${agent}: no memory dir — not swept (no session has run here)`,
      });
      continue;
    }
    ...
    if (bytes === null) {
      const message =
        `${agent}: index MISSING or unreadable at ${path ?? "(unknown path)"} — the memory dir ` +
        `exists, so sessions run here with NO index at all. An unmeasurable agent cannot be ` +
        `reported as under budget.`;
      out.push({
        agent, path, bytes: null,
        state: MEMORY_SIZE_STATES.MISSING,
        overBy: null, headroom: null, message,
      });
      findings.push({ type: "memory-index-missing", severity: "warn", agent, path, message });
      continue;
    }

    if (bytes > budget) {
      const overBy = bytes - budget;
      const message =
        `${agent}: index is ${bytes} bytes vs the ${budget}-byte cap — ${overBy} over; ` +
        `${LOADER_TRUNCATION_SENTENCE}.`;
      out.push({
        agent, path, bytes,
        state: MEMORY_SIZE_STATES.OVER,
        overBy, headroom: 0, message,
      });
      findings.push({
        type: "memory-index-over-budget",
        severity: "warn",
        agent, path, bytes, budgetBytes: budget, overBy, message,
      });
      continue;
    }
    ...
    out.push({
      agent, path, bytes,
      state: MEMORY_SIZE_STATES.OK,
      overBy: null, headroom: budget - bytes, message: null,
    });
  }

  const countOf = (state) => out.filter((r) => r.state === state).length;
  const sweptCount = countOf(MEMORY_SIZE_STATES.OK) + countOf(MEMORY_SIZE_STATES.OVER);
  return {
    budgetBytes: budget,
    rows: out,
    findings,
    sweptCount,
    declaredCount: countOf(MEMORY_SIZE_STATES.NO_MEMORY_DIR),
    overCount: countOf(MEMORY_SIZE_STATES.OVER),
    missingCount: countOf(MEMORY_SIZE_STATES.MISSING),
    malformedCount,
    verdict: findings.length > 0 ? "findings" : sweptCount === 0 ? "nothing-swept" : "clean",
  };
}
