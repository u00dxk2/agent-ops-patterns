...
export function lintMemoryIntegrity({ indexText, files, indexByteLength, budgetBytes } = {}) {
  ...
  const fileList = rawList.filter(
    (f) => f && typeof f.name === "string" && f.name.toLowerCase() !== "memory.md",
  );
  const skippedCount = rawList.length - fileList.length;
  ...
  // nothing must be legible as such. `reached` counts topic files actually
  // linted; `skipped` counts entries dropped for being malformed or for being
  // the index itself. `swept` is false when nothing was reachable at all -
  // check it BEFORE you read findings.
  const swept = fileList.length > 0 || Boolean(indexText);
  return {
    findings,
    swept,
    coverage: {
