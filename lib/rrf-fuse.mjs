 ...
 * Σ 1/(k + rank_i) across the rankings, with ONE-BASED ranks and k = 60, as in
 * Cormack, Clarke & Buettcher (SIGIR 2009). The indexing matters and used to be
 * wrong here: zero-based ranks make the top term 1/60 instead of 1/61, which is
 * the paper's formula at k=59 and flips close cases. RRF
...
export function rrfFuse(pool, limit, k = RRF_K) {
  ...
  return pool
    // Ranks are ONE-based, as in the paper. They used to be zero-based, which
    // made the top term 1/60 rather than 1/61 — arithmetically the paper's
    // formula at k=59, not k=60, and enough to flip close cases. The citation
    // was right about the constant and wrong about the indexing.
    .map((item, i) => ({ item, i, rrf: 1 / (k + i + 1) + 1 / (k + /** @type {number} */ (scoreRank.get(i)) + 1) }))
