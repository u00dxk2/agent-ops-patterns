...
export function matchGrant(grants, query = /** @type {any} */ ({}), allowedClasses = []) {
  ...
  if (typeof query.nowMs !== "number" || !Number.isFinite(query.nowMs)) return null;
  if (typeof query.command !== "string") return null;
  const command = query.command; // captured once; hashed once
  if (!normalizeCommand(command)) return null;
  const wantHash = commandHash(command);
  const wantScope = typeof query.scope === "string" ? query.scope.trim() : "";
  const wantClass = typeof query.actionClass === "string" ? query.actionClass.trim() : "";
  if (!wantScope || !wantClass) return null;

  for (const g of grants) {
    if (!isGrantLive(g, query.nowMs)) continue;
