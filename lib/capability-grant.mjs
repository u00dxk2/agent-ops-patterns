...
export function isAllowedGrantClass(cls, allowedClasses) {
  if (!Array.isArray(allowedClasses)) return false;
  ...
  return typeof cls === "string" && allowed.includes(cls.trim());
}
...
export function buildGrant(opts = /** @type {any} */ ({})) {
  ...
  const allowedClasses = Array.isArray(opts.allowedClasses) ? Array.from(opts.allowedClasses) : opts.allowedClasses;
...
export function parseGrant(raw, allowedClasses) {
  ...
  // `j` comes from JSON.parse, so it has no accessors today — but this function
  // is one rename away from being handed a live object, and the same
  // validate-then-re-read shape is what let an accessor through elsewhere in
  // this file. Snapshot once, validate the snapshot, return the snapshot.
  const g = {
    id: j.id,
    command: j.command,
    commandSha256: j.commandSha256,
    scope: j.scope,
    actionClass: j.actionClass,
    mintedAtMs: j.mintedAtMs,
    expiresAtMs: j.expiresAtMs,
    singleUse: j.singleUse,
    mintedBy: j.mintedBy,
    consumedAtMs: j.consumedAtMs,
  };
  if (typeof g.commandSha256 !== "string" || typeof g.scope !== "string" || typeof g.actionClass !== "string") {
    return null;
  }
  if (!isAllowedGrantClass(g.actionClass, allowedClasses)) return null;
  return {
    v: GRANT_SCHEMA_VERSION,
    id: g.id,
    command: g.command,
    commandSha256: g.commandSha256.toLowerCase(),
    scope: g.scope.trim(),
    actionClass: g.actionClass.trim(),
    mintedAtMs: g.mintedAtMs,
    expiresAtMs: g.expiresAtMs,
    singleUse: g.singleUse !== false,
    mintedBy: typeof g.mintedBy === "string" ? g.mintedBy : undefined,
    consumedAtMs: g.consumedAtMs,
  };
}
...
export function isGrantLive(grant, nowMs) {
  ...
  // Read the two decision fields ONCE. hasValidShape already read them to
  // confirm the shape; re-reading here let an `expiresAtMs` getter return a
  // finite future timestamp to the validator and NaN to the comparison —
  // `nowMs >= NaN` is false, so an expired grant read as live.
  const expiresAtMs = grant.expiresAtMs;
  const consumedAtMs = grant.consumedAtMs;
  if (typeof expiresAtMs !== "number" || !Number.isFinite(expiresAtMs)) return false;
  if (nowMs >= expiresAtMs) return false;
  if (consumedAtMs !== undefined) return false;
  return true;
}
