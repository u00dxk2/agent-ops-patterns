...
export function screenDecision(input) {
  ...
  // Read every field EXACTLY ONCE, before deciding anything. `flagged` used to
  // be read twice — once for the typeof gate, once to pick the verdict — so an
  // accessor-backed input returning true then false was type-checked as a real
  // finding and then enforced as `allow`. Same defect class as the accessor
  // TOCTOU in capability-grant; found by sweeping for the pattern rather than
  // by fixing the one instance we already knew about.
  const flagged = o.flagged;
  const shadow = o.mode === "shadow";
  const ran = o.ran === true && typeof flagged === "boolean";
  const unscreenedProceeds = o.unscreenedProceeds === true;

  if (!ran) {
    return { verdict: "unscreened", proceed: shadow ? true : unscreenedProceeds };
  }
  if (shadow) {
    return flagged ? { verdict: "would_block", proceed: true } : { verdict: "shadow_allow", proceed: true };
  }
  return flagged ? { verdict: "block", proceed: false } : { verdict: "allow", proceed: true };
}
