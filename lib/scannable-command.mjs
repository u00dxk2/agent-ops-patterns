 ...
 *   1. ANSI-C $'…' wrappers unwrapped and their escapes decoded — the named
 *      ones (\n \t \r \a \b \e \f \v \\ \' \" \?), plus hex (\xHH), octal
 *      (\nnn), unicode (\uHHHH, \UHHHHHHHH) and control (\cX) spellings;
...
function decodeAnsiC(inner) {
  return inner.replace(
    ...
    (m, /** @type {string} */ seq) => {
      ...
      if (head === "c") return String.fromCharCode(seq.charCodeAt(1) & 0x1f); // control char
      if (/^[0-7]{1,3}$/.test(seq)) return String.fromCharCode(parseInt(seq, 8));
