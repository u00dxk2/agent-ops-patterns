...
A search that finds nothing is evidence only in this form: the directory it ran from, the exact command, its exit code, and the "0 matches" line. Empty output with none of those is not evidence - it cannot be told from a wrong directory or a failed search. **The glob is part of the receipt**: record it, and run the same categories over prompt/skill/workflow directories, not just source files - in a skills-based ecosystem, markdown-defined workflows are code, and a source-only glob inventories half the system (a judge ensemble hid from one run exactly this way). A surface you could not reach (a submodule you cannot read, a deploy you cannot see, a command that errored) is **UNREADABLE**, and is never rounded to DOES NOT APPLY.

Inventory by search, not by reading: on a large repo the evidence bar is satisfiable from grep hits plus a bounded read around each - never open a megabyte file whole. A config setting that nothing reads is evidence, not a shrug.
...
**3. Pre-commit the bars.** Some verdicts turn on a number the run has to *compute* - a backtest, a precision, a spread over history. For each, write the threshold and what happens on either side of it before you compute the number, in the "Bars pre-committed" section. The exception to step 2's freedom: when a verdict turns on an inventory count itself - "is this label set big enough?", "is this history long enough?" - write the bar *before* you count. A number you meet while inventorying counts as read - a ceiling you saw in a README you opened for the inventory cannot get a pre-committed bar afterward, only a disclosed late one. If you find you computed first and decided after, say so in the row, and count it in the summary line "Bars written after their number." That line is part of the report; a run with a nonzero count there is weaker, and says so, which is better than a run that hides it.

**4. Judge each idea.** Open `references/disposition-template.md` and fill the table. Five verdicts, each with a quoted `file:line` or the negative-search evidence in the form above. When the claim is about what code DOES, the receipt is the doing line, not the docstring - a header can assert the opposite of the body, and in one audited run it did:
...
**7. Only now open the answer keys: `references/answer-keys/worked-example.md`, then `references/answer-keys/worked-example-2.md`.** The first is the run on the system this skill was written for; the second is the run on a large open-source agent framework nobody here owns. They live in their own quarantined folder (its `answer-keys/README.md` is the door sign) because two runs in a row opened them early while learning the skill's file layout: they contain verdicts, and if the repo you were pointed at is either system or one shaped like it, they are answer keys. Read them after your table is filled, to check your *form* against them - evidence per row, bars before numbers, what was not checked - never to check your verdicts.
...
| `answer-keys/README.md` | The door sign on the quarantined folder - why nothing in it opens before step 7. |
| `answer-keys/worked-example.md` | **Step 7 only, after your table is filled.** Contains verdicts for one real system; read for form, not answers. |
| `answer-keys/worked-example-2.md` | **Step 7 only, after your table is filled.** The same skill run read-only against a 10k-file open-source agent framework; verdicts for that system, the cross-family round that overturned two of them, and ten notes on what the skill got wrong when pointed at a stranger's repo. |
...
- The record it produces has not been refuted by anyone when the run ends. On the system this was written for, adversarial verifiers found defects in three of four builder receipts, a cold second reader found three gaps the first run missed, and a Codex review of the skill itself found sixteen. The second worked example - a stranger's repo, one session, read-only - named the row it thought most likely wrong; a cross-family round then overturned two OTHER rows and left that one standing, which is idea 8 applied to the record itself. Assume yours will fare the same. Have someone refute your record before you act on it.
