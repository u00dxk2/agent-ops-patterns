...
**7. Only now open `references/worked-example.md`, and then `references/worked-example-2.md`.** The first is the run on the system this skill was written for; the second is the run on a large open-source agent framework nobody here owns. Both are placed last deliberately: they contain verdicts, and if the repo you were pointed at is either system or one shaped like it, they are answer keys. Read them after your table is filled, to check your *form* against them - evidence per row, bars before numbers, what was not checked - never to check your verdicts.
...
| `worked-example-2.md` | **Step 7 only, after your table is filled.** The same skill run read-only against a 10k-file open-source agent framework; verdicts for that system, and ten notes on what the skill got wrong when pointed at a stranger's repo. |
