...
**Its verdicts:** APPLIES 2 · DOES NOT APPLY 1 · ALREADY IN PLACE 5 (all of them code-path evidence; the running/code split did not exist yet) · NOT DECIDABLE 2, on the ten-row template of the time. Acted on: 0, by mandate.
...
**Its own caveat, kept verbatim in spirit:** it had read the answer key before drafting, reached most verdicts independently, and could not honestly claim its idea-2 and idea-3 rows were uncontaminated. That is why the gating changed - and why "cold" is the wrong word for that run: no memory of the system, yes; uncontaminated, no. And nobody had refuted its record when it finished - the same line it now asks every run to end with.

## The Codex adversarial round (2026-08-28, same day, after the fixes above)

A second model family was pointed at the revised skill with the instruction to find ways it would make an agent produce a wrong, unverifiable, or misleading record. It returned sixteen findings, every one with line numbers, none on security. Fifteen were taken as written; one was adapted. What changed:

- **A read-only mandate still wrote a file.** The procedure distinguished read-only from record-only and then told both to write the record into the repo. Now: four mandate words, defined; under read-only the record is delivered in the reply or wherever the operator names outside the repo; the record write never counts as "acted on."
- **ALREADY IN PLACE meant two different things** - a code path exists, and a code path runs - and the summary counted them as one. Now tagged **(code)** or **(running)** and counted separately; most runs will earn few of the second, and the skill says so.
- **APPLIES and NOT DECIDABLE overlapped**: "no record exists" is a gap (APPLIES) and a missing measurement (NOT DECIDABLE) at once, so two agents could count the same evidence differently. Precedence added: if the only gap is the missing record, it is NOT DECIDABLE and the record is the next step.
- **A negative search had no satisfiable evidence form** - "the command and its output line," when the output is empty, cannot be told from a wrong directory or a failed command - and nothing could express "I could not reach this." Now: directory, exact command, exit code, "0 matches" line; and a fifth verdict, **UNREADABLE**, never rounded to DOES NOT APPLY.
- **The inventory had no rows for three of the ideas' surfaces** (candidate generation and selection, pre-execution review points, model routing per call site), so an agent could complete every bullet and still judge ideas 1, 5a, 9a and 10 unevidenced. Three rows added.
- **Inventory counts were exempt from bars even when a verdict turned on the count** ("is the label set big enough?"). The exemption now excludes exactly that case. The adapted finding: Codex proposed that a bar written after its number should invalidate the run and force a rerun; the skill instead keeps disclosure, but moves it from the row to a counted line at the top of the summary - a nonzero count there is a weaker run that says so, which we judged better than an incentive to hide or to re-run until the number cooperates.
- **Idea 8 cited "part 1" for a claim no paper in the manifest backs** (models prefer their own traces). Now labelled a lecturer's Q&A remark, treated as anecdote. **LATS was mapped to idea 10**, which never invokes it; now `-`, cited only from the "argues against" list. **Idea 1 routed same-model self-ranking to idea 2 alone**; now to 2 and 8. **Idea 2's ALREADY IN PLACE question tested filtering only**; now filtering, weighting, and whether the judges ever disagree.
- **Three test headers claimed more than their assertions checked** (a "well-formed frontmatter" regex; a gating check that only proved the words appeared somewhere; a reference-file scan that missed links and most filenames). Each assertion now matches its header: exact key set, first-mention-after-step-5 ordering, backticks and links both ways.
- **Public copy said "cold" for a contaminated run, "eight" for ten complaints, "every paper the course teaches" for the videos' set, and "a quoted path per verdict" in two manifests and a README row after the template had already dropped that promise.** All four corrected.

The round's own lesson, for the record: the skill had by then been revised twice by its author on the strength of one outside run, and a different model family still found sixteen things in an hour. The last line of every disposition - "refuted by: nobody yet" - is there because of days like this one.
