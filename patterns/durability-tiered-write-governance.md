...
Place every action an agent can take on a three-rung ladder, and put the governance where the durability is. The thinking upstream of this is Cliff Rosen's, at Orchestrator Studios - ["The Agent in the Middle"](https://orchestratorstudios.com/articles/the-agent-in-the-middle.html) separates the durable substrate from the UX layer frozen on top of it, and ["The Living Workspace"](https://orchestratorstudios.com/articles/living-workspace.html) organizes workspace components around presentation (views), access (tools), understanding (schemas and capabilities), and substrate (data). The three-rung ladder below is not his tiering and shouldn't be read as his - it's our operational adaptation, plus the tests that make each rung enforceable rather than aspirational.

## The ladder

### Rung 1 — Reads. Never gated.

Deploy-status queries, log reads, metric pulls, campaign reads. Governance here is *authentication* (who may see), never *approval* (may this happen). The enforceable rule: **no approval gate may fire on an effect-free, already-authorized read** - every approval pattern in your system should require a mutation verb. Note what that rule does not say. Reads still answer to access control, and some of them need more than that: a read can disclose data the caller shouldn't see, cost real money, exhaust a rate limit, or hit an endpoint that only calls itself a read. Those still get gated - by authentication, cost policy, or a rule of their own. What you're removing is the per-action human click on "check the deploy status," because the day that prompt fires you have taxed the thing agents do hundreds of times a day and trained your operator to rubber-stamp.

### Rung 2 — Schema-bounded writes. Machine-approved, human-informed.

Writes inside shapes the system already owns: a git push that auto-deploys behind CI, a message appended to a bus, an application-level write through existing code paths. The property that defines this rung is **reversibility by another write of the same shape** - a bad push is reverted by a push, a bad row by an update. Two conditions, and they're easy to assume rather than check: the prior state has to still exist somewhere (an update over an unversioned row is not reversible, it's gone), and the downstream consequences have to be reversible too. A push that auto-deploys can page a customer, run a migration, or leak something - and reverting the commit undoes none of that. When either condition fails, the action isn't rung 2. Classify it by its furthest irreversible consequence, which is the rule the rest of this document runs on.
