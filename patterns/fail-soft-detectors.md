...
The operational failure modes of LLM-agent systems — silent stalls, wedged listeners, dead schedulers, half-delivered work — are boring, mechanical, and constant. The instinct is to point another LLM at them ("a watchdog agent"). That instinct is wrong, and this document is the discipline that replaces it, extracted from a system that runs dozens of concurrent coding-agent sessions across a 22-product portfolio with one human operator.
...
- **Presence is the whole scope, and that is a real ceiling.** Rule 1 buys cheap, trustworthy, deterministic monitoring by giving up on quality entirely. A fleet whose every heartbeat is fresh, whose every queue drains, and whose every agent produces confident garbage reads perfectly green here. These detectors tell you the machine is running, never that it is doing the right thing.
- **Fail-soft under-reports on purpose.** Rule 2's default resolves enumerated ambiguity to silence, so a genuine fault wearing an ambiguous shape is missed by design. That trade is correct when false alarms cost you an operator's attention; it is wrong in a domain where a miss costs more than a wasted page. Invert the default there, and say so in the header.
- **A posture is not a guarantee, and §2 says so for a reason.** Declared postures describe how *declared* cases are handled. An implementation bug is not a declared case, and this repo's own history — a grant that authorized the wrong command, a redactor that returned a raw secret while reporting success — is what that distinction is made of. Read a posture as a reviewable intent, never as a safety property.
- **Written from a one-operator fleet.** The dedup policy (rule 4) and the per-detector kill-switch (rule 7) both assume a single human decides what is noise and what gets turned off. At team scale each needs an owner and an escalation path, neither of which this pattern supplies.
