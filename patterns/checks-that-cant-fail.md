...
**The guard:** a check that swept zero targets must report **NOTHING SWEPT**, never "clean" — they are different verdicts and only one of them is reassuring. Count what you actually reached (files opened, projects enumerated, rows scanned) and surface that count in the result. Zero reached is a probe failure to escalate, not a pass - but zero is only the loudest case, and stopping there leaves the quiet one open. Reaching one target out of ten thousand also returns "clean." So report four numbers, not one: how many subjects you **expected**, how many you **reached**, how many you **skipped**, and how many **errored**. Then reject the run when reached is zero *or* when coverage falls below a threshold you declared in advance. A checker that silently drops malformed entries and reports the survivors as clean is the same bug wearing better clothes. Search by content across the whole tree and exclude by explicit path (`.git/`, `node_modules/`); never define your search by an inclusion pattern that silently amputates the riskiest inputs.
...
- **A red proof is a point-in-time fact about one version.** `redProvenAt` says the check fired on that input, on that day, against that code. A later refactor can sever the path and the timestamp stays as green as ever — it is evidence that ages, and nothing here notices it aging. Re-arm on change, not on a calendar.
- **A positive control proves the path, not the population.** Planting a fake secret shows the scanner can flag *that shape*. It says nothing about the shapes you never planted, and a probe can hold a passing positive control while remaining blind to most of what you care about. This guard bounds instrument-death; it does not measure coverage.
- **None of the four modes tells you the condition is the right one.** Every guard here is about whether a check *can speak*. A check that fires reliably, on a threshold that is simply wrong, passes all four and misleads you with full mechanical integrity. Correctness of the predicate is a separate review.
- **The discipline costs real engineering per check.** Four coverage numbers, a written red condition, a positive control in the same code path — on a check whose failure is cheap, loud and quickly noticed anyway, that overhead can exceed the bug it prevents. Spend it where a silent green would cost you something.
